"""Unit test for the permission-marshaling path.
...
queue. This test exercises both single-prompt round-trips and the
multi-prompt routing.

Run with:

    .venv/bin/python -m tests.test_permission_handler
"""
...
def main() -> None:
    ...
    result: dict = {}
    t = threading.Thread(
        target=lambda: result.__setitem__("ok", state.permission_handler(target)),
        daemon=True,
    )
    t.start()
    ...
    assert target.resolve() not in permissions.approved_paths(), "deny should not cache"
    ...
    assert (
        target.resolve() not in permissions.approved_paths()
    ...
    assert target.resolve() in permissions.approved_paths(), "'always' should cache"
    ...
    decision: dict = {}
    t = threading.Thread(
        target=lambda: decision.__setitem__("ok", permissions.require_access(outside)),
        daemon=True,
    )
    t.start()
    assert _wait_for(lambda: bool(fake.sent))
    ...
    result_b: dict = {}
    ta = threading.Thread(
        target=lambda: result_a.__setitem__("ok", state.permission_handler(target_a)),
        daemon=True,
    )
    tb = threading.Thread(
        target=lambda: result_b.__setitem__("ok", state.permission_handler(target_b)),
        daemon=True,
    )
    ta.start()
    tb.start()
