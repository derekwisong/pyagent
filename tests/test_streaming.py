"""End-to-end test for the streaming hook on the LLMClient protocol.
...
Run with:
    ...
    .venv/bin/python -m tests.test_streaming
"""
...
def _check_gemini_streaming_mocked() -> None:
    ...
    os.environ.setdefault("GEMINI_API_KEY", "dummy-for-test")
    from pyagent.llms.gemini import GeminiClient
    ...
    one_shot = _Attr(
        candidates=[
            _Attr(content=_Attr(parts=[_Attr(text="non-stream", function_call=None)]))
        ],
        usage_metadata=_Attr(
...
def main() -> None:
    ...
    print("test_streaming: all checks passed")
