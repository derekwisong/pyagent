...
Run with:
    ...
    .venv/bin/python -m tests.test_submit_handler
"""
...
def main() -> None:
    ...
    print("✓ /perms lists pending requests; head marked active")
    ...
    assert visible in _SPINNER_FRAMES, f"unexpected spinner glyph {visible!r}"
    ...
    try:
        ...
        assert (
            state.agent.pending_async_replies.qsize() == 0
        ), "idle-window note polluted inbox"
        print(
            f"✓ idle window → user_note → promoted to user_prompt: "
            f"{promoted['prompt']!r}"
        )
