"""End-to-end test for the code-mapper plugin.
...
Run with:
    ...
    .venv/bin/python -m tests.test_code_mapper
"""
...
def _check_docstrings_optin() -> None:
    ...
    print("✓ include_docstrings=True attaches docstrings; default omits them")
...
def _check_unsupported_extension() -> None:
    ...
    out_str = mapper.map_code_for_path("/tmp/whatever.foo", b"junk", kind="all")
    payload = json.loads(out_str)
    assert "error" in payload, payload
    assert ".foo" in payload["error"], payload["error"]
    print("✓ unsupported extension → clean error payload")
...
def _check_end_to_end_against_real_file() -> None:
    ...
    if not target.exists():
        ...
        return
    out_str = mapper.map_code_for_path(str(target), target.read_bytes(), kind="classes")
    payload = json.loads(out_str)
...
def _check_map_rust() -> None:
    ...
    print("✓ Rust: struct/enum/trait/method/function classified correctly")
...
def _check_map_c() -> None:
    ...
    print("✓ C: struct/typedef/enum/union/function classified correctly")
...
def _check_map_cpp() -> None:
    ...
    speak_in_animal = [
        s
        for s in out["symbols"]
        if s["kind"] == "method" and s["name"] == "speak" and s["parent"] == "Animal"
    ]
    assert len(speak_in_animal) == 2, speak_in_animal
    # Free function inside namespace.
    assert has("function", "top", "app"), out["symbols"]
    print("✓ C++: class/struct/method (in-class + out-of-line)/namespace OK")
...
def _check_map_typescript() -> None:
    ...
    print("✓ TypeScript: class/method/interface/enum/type/function/module")
...
def _check_map_tsx() -> None:
    ...
    print("✓ TSX: JSX functions / class component / arrow component")
...
def _check_map_go() -> None:
    ...
    print("✓ Go: package/struct/interface/type/const/var/method/function")
...
def _check_map_java() -> None:
    ...
    print("✓ Java: class/field/method/constructor/interface/enum/record")
...
def _check_map_bash() -> None:
    ...
    print("✓ Bash: function + variable_assignment")
...
def _check_map_ruby() -> None:
    ...
    print("✓ Ruby: module/class/method (incl. singleton methods)")
...
def _check_map_json() -> None:
    ...
    print("✓ JSON: top-level + nested fields with parent attribution")
...
def _check_map_yaml() -> None:
    ...
    print("✓ YAML: nested mapping keys with parent attribution")
...
def _check_map_toml() -> None:
    ...
    print("✓ TOML: section headers (incl. dotted) + pairs with attribution")
...
def _check_map_markdown() -> None:
    ...
    print("✓ Markdown: H1-H3 headings, H4+ excluded")
...
def _check_map_dockerfile() -> None:
    ...
    print("✓ Dockerfile: stages + per-instruction directives")
...
def _check_map_swift() -> None:
    ...
    assert has("function", "top", None), out["symbols"]
    print("✓ Swift: class/property/method/protocol/function")
...
def _check_map_kotlin() -> None:
    ...
    assert has("function", "top", None), out["symbols"]
    print("✓ Kotlin: class/method/object/function")
...
def _check_map_scala() -> None:
    ...
    print("✓ Scala: package/class/object/trait/method/function")
...
def _check_map_lua() -> None:
    ...
    print("✓ Lua: function (plain/dotted/local/table) + method (colon)")
...
def _check_map_php() -> None:
    ...
    assert has("function", "top", None), out["symbols"]
    print("✓ PHP: class/method/interface/trait/function")
...
def _check_map_css() -> None:
    ...
    print("✓ CSS: class/id selectors + at-rules (@media / @keyframes / @font-face)")
...
def _check_map_sql() -> None:
    ...
    print("✓ SQL: table/view/index/function/schema/trigger")
...
def _check_probe_grammar() -> None:
    ...
    out = mapper.probe_grammar("python", "a=1\nb=2\nc=3\nd=4\ne=5", max_nodes=4)
    ...
    print("✓ probe_grammar: field names visible, bounded, language-pack-direct")
...
def main() -> None:
    ...
    print("test_code_mapper: all checks passed")
