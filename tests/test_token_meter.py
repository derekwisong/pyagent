...
No subprocess, no real network. Run with:
    ...
    .venv/bin/python -m tests.test_token_meter
"""
...
def _check_cache_token_aggregation() -> None:
    ...
    cost = _estimate_cost_usd("anthropic/claude-sonnet-4-6", 1000, 500, 1000, 500)
    ...
    _update_agents_state(agents, {"type": "usage", "input": 10, "output": 5})
    ...
    anth = _format_usage_suffix(100, 50, "anthropic/claude-sonnet-4-6", 200, 1000)
...
def main() -> None:
    ...
    _update_agents_state(agents, {"type": "usage", "input": 100, "output": 50})
    assert agents["root"]["tokens"] == {
        "input": 100,
        "output": 50,
        "cache_creation": 0,
    ...
    _update_agents_state(agents, {"type": "usage", "input": 200, "output": 80})
    assert agents["root"]["tokens"] == {
        "input": 300,
        "output": 130,
        "cache_creation": 0,
...
def _check_format_right_zone() -> None:
    ...
    gross_int, net_float = gross_net_tokens(1000, 500, "openai/gpt-4o", 0, 1000)
