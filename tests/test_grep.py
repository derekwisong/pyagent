...
Run with:
    ...
    .venv/bin/python -m tests.test_grep
"""
...
def main() -> None:
    tmp = Path(tempfile.mkdtemp(prefix="pyagent-test-grep-")).resolve()
    ...
    print("[ok] context=2 equals before=2,after=2")
    ...
    print("[ok] context truncates at file boundaries")
    ...
    print("[ok] non-adjacent matches separated by --")
    ...
    print("[ok] multiple files emit `--` between groups")
    ...
    assert "--" not in out, out
    print("[ok] no-context multi-file output stays flat")

    # 9. Bad input: negative integer / non-coercible string.
    out = grep(r"return", fp, before=-1)
    assert (
        len(out) == 1 and out[0].startswith("<error:") and "non-negative" in out[0]
    ), out
