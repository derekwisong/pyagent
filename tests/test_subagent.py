"""End-to-end test for spawn_subagent / call_subagent / terminate_subagent.
...
Run with:
    ...
    .venv/bin/python -m tests.test_subagent
"""
...
def main() -> None:
    tmp = Path(tempfile.mkdtemp(prefix="pyagent-subagent-test-"))
    os.chdir(tmp)
    print(f"cwd: {tmp}")
    ...
    spawn = subagent.make_spawn_subagent(state, agent, parent_session, base_config)
    call = subagent.make_call_subagent(state, agent)
    terminate = subagent.make_terminate_subagent(state, agent)

    io_thread = threading.Thread(target=state.io_loop, name="test-io", daemon=True)
    io_thread.start()
    ...
    try:
        ...
        assert any(
            e.get("type") == "info" and "spawned" in e.get("message", "")
            for e in events
        ), events
        assert "assistant_text" in kinds, kinds
        assert any(
            e.get("type") == "info" and "terminated" in e.get("message", "")
            for e in events
        ), events
