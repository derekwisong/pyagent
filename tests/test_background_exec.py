"""Unit test for the background-shell tools.
...
Run with:
    ...
    .venv/bin/python -m tests.test_background_exec
"""
...
def _check_run_and_wait_exit() -> None:
    ...
    out = agent_tools.run_background("echo hello-bg && exit 0", name="quick-echo")
...
def _check_output_contains() -> None:
    ...
    started = agent_tools.run_background("(sleep 0.2; echo READY-MARKER; sleep 5)")
    handle = re.search(r"started (bg-[0-9a-f]+)", started).group(1)
    t0 = time.monotonic()
...
def _check_silence_wait() -> None:
    ...
    # the next line).
    started = agent_tools.run_background("(echo a; echo b; echo c; sleep 5; echo d)")
    handle = re.search(r"started (bg-[0-9a-f]+)", started).group(1)
    t0 = time.monotonic()
    res = agent_tools.wait_for(handle, until="silence:0.5s", timeout_s=4.0)
    elapsed = time.monotonic() - t0
...
def _check_kill_active_flushes_both() -> None:
    ...
    # Foreground sleep on a worker thread (mirrors test_kill_active).
    fg_result: dict = {}
...
def _check_buffer_cap_truncation() -> None:
    ...
    assert bg.dropped > 0, f"expected output drops, got dropped={bg.dropped}"
    # And the buffer itself should be near (≤) the cap.
    assert (
        len(bg.output_buf) <= 1024 * 1024
    ...
    second = agent_tools.read_output(handle, since=next_since, max_chars=200)
    assert (
        "...truncated" not in second
...
def _check_dual_stream_read_output() -> None:
    ...
    for marker in ("OUT1", "OUT2", "OUT3", "ERR1", "ERR2", "ERR3"):
        assert marker in full, f"expected {marker} in combined output, got: {full!r}"
...
def _check_shutdown_background_grace() -> None:
    ...
    started = agent_tools.run_background("trap '' TERM; sleep 30")
    handle = re.search(r"started (bg-[0-9a-f]+)", started).group(1)
    t0 = time.monotonic()
