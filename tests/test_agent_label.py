...
Run with:
    ...
    .venv/bin/python -m tests.test_agent_label
"""
...
def main() -> None:
    ...
    buf = io.StringIO()
    Console(file=buf, force_terminal=False, color_system=None).print(f"{label}ready")
    ...
    rendered = buf.getvalue().rstrip()
    assert rendered == "[sleeper-519f719d] · execute  command=sleep 5", repr(rendered)
