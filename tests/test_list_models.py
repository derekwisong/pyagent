"""End-to-end test for `pyagent --list-models` and the
...
Run with:
    ...
    .venv/bin/python -m tests.test_list_models
"""
...
def _check_failing_provider_renders_error() -> None:
    ...
    _check(
        ...
        all(l.name in by_name for l in listings if l.name != llms.PROVIDERS[0].name),
    )
...
def _check_plugin_list_models_hook_plumbs_through() -> None:
    ...
    _check(
        ...
        rec.list_models is not None and rec.list_models() == sentinel_models,
        repr(rec),
    )
...
class _FakeResponse:
    ...
    error-path live in `test_ollama_plugin.py`; this fixture only
...
def _check_cli_prints_and_exits() -> None:
    ...
    runner = CliRunner()
    tmp = Path(tempfile.mkdtemp(prefix="pyagent-test-listmodels-"))
...
def main() -> None:
    ...
    print("test_list_models: all checks passed")
