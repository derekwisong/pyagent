"""End-to-end test for file-based markdown roles.
...
Run with:
    ...
    .venv/bin/python -m tests.test_roles_md
"""
...
def _check_legacy_models_table_still_resolves_and_warns(tmp: Path, caplog) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger="pyagent.roles"):
        roles.load()
    assert not any(
        "deprecated" in r.getMessage() for r in caplog.records
...
def _check_cli_list_runs(tmp: Path, user_dir: Path) -> None:
    ...
    # Project-tier
    _write(
        tmp / ".pyagent" / "roles" / "proj_only.md", "# Role: Proj\n\nProject role.\n"
    )
    runner = CliRunner()
...
def _check_cli_init_idempotent(tmp: Path, user_dir: Path) -> None:
    ...
    assert "wrote" not in second.output.lower().replace("(none)", "").split("done:")[0]
...
class _CapLog:
    """Tiny pytest-style caplog stand-in for the test harness."""
...
def main() -> None:
    tmp = Path(tempfile.mkdtemp(prefix="pyagent-roles-md-test-"))
    user_dir = Path(tempfile.mkdtemp(prefix="pyagent-roles-md-userdir-"))
    os.chdir(tmp)
    print(f"cwd: {tmp}")
    print(f"config-dir (mocked): {user_dir}")

    # Stub paths.config_dir() for the duration of the test run so the
