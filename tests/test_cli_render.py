...
Run with:
    ...
    .venv/bin/python -m tests.test_cli_render
"""
...
def _check_tool_result_renders_success_and_error() -> None:
    ...
    cli._turn_tool_calls[:] = [{"name": "glob", "ok": True}]
    with mock.patch.object(cli.console, "print") as fake_print:
        cli._on_tool_result("glob", "Error: no matches\n", agent_id=None)
    line = fake_print.call_args.args[0]
    _check(
...
def _check_turn_summary_renders_and_clears() -> None:
    ...
    _check(
        ...
        "create_memory ✓" in line and "glob ✗" in line and "execute ✓" in line,
        repr(line),
    )
    _check(
...
def main() -> None:
    ...
    print("test_cli_render: all checks passed")
