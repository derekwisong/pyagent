"""End-to-end test for config-defined roles.
...
Run with:
    ...
    .venv/bin/python -m tests.test_roles
"""
...
def test_set_model_handler() -> None:
    ...
    assert isinstance(
        state.agent.client, LoremClient
...
def main() -> None:
    tmp = Path(tempfile.mkdtemp(prefix="pyagent-roles-test-"))
    os.chdir(tmp)
    print(f"cwd: {tmp}")
