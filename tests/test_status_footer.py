...
Run with:
    ...
    .venv/bin/python -m tests.test_status_footer
"""
...
def check_state_machine() -> None:
    ...
    _update_agents_state(agents, {"type": "weird_unknown_event_type"})
...
def check_tier_collapse() -> None:
    ...
    print("✓ tier C error bucket only when non-zero")
...
def check_spinner_predicate() -> None:
    ...
    assert _tree_busy({"root": {"status": "ready"}, "s1": {"status": "· bash"}}) is True
    # Mixed: root working, all subs idle → busy.
    assert (
        _tree_busy({"root": {"status": "thinking"}, "s1": {"status": "ready"}}) is True
    )
    # All terminal → not busy.
    assert _tree_busy({"root": {"status": "ready"}, "s1": {"status": "error"}}) is False
...
def check_right_zone_contract() -> None:
    ...
    out = compose_plain(agents, "anthropic", cols=100)
    assert len(out) == 100, (len(out), out)
    print("✓ pad math: len matches cols for normal usage")
