...
Run with:
    ...
    .venv/bin/python -m tests.test_controlling_hooks
"""
...
# ---- Test fixture helpers (mirrored from tests/test_plugins.py) ----
...
def test_extra_user_message_before_and_after() -> None:
    ...
    try:
        ...
        assert any(
            n == "[plugin notes notes]: heads up: tool incoming" for n in notes
        ), notes
        assert any(
            n == "[plugin notes notes]: heads up: tool ran" for n in notes
        ), notes
        print(
...
def test_block_beats_mutate() -> None:
    ...
    try:
        ...
        _write_plugin(
            cfg / "plugins", dirname="01-a", name="block-a", plugin_py=plugin_a
        )
        _write_plugin(
            cfg / "plugins", dirname="02-b", name="mutate-b", plugin_py=plugin_b
        )
        loaded = plugins_mod.load()
        agent = _mk_agent(loaded)
        result = agent._route_tool(
...
def test_mutate_chaining() -> None:
    ...
    try:
        ...
        _write_plugin(
            cfg / "plugins", dirname="01-mA", name="mut-a", plugin_py=plugin_a
        )
        _write_plugin(
            cfg / "plugins", dirname="02-mB", name="mut-b", plugin_py=plugin_b
        )
        loaded = plugins_mod.load()
        agent = _mk_agent(loaded)
        result = agent._route_tool(
...
def test_replace_result_chaining() -> None:
    ...
    try:
        ...
        _write_plugin(
            cfg / "plugins", dirname="01-rA", name="rep-a", plugin_py=plugin_a
        )
        _write_plugin(
            cfg / "plugins", dirname="02-rB", name="rep-b", plugin_py=plugin_b
        )
        loaded = plugins_mod.load()
        agent = _mk_agent(loaded)
        result = agent._route_tool(
...
def test_strategic_reevaluation_path_keyed() -> None:
    ...
    try:
        ...
        assert (
            "strategic-reevaluation" in names
        ...
        call_edit("/a")
        notes = _drain_pending(agent)
        assert notes == [], f"only 2 failures yet; should not have fired: {notes}"
        call_edit("/a")
        notes = _drain_pending(agent)
        assert any("strategic-reevaluation notes" in n for n in notes), notes
        assert any("/a" in n for n in notes), notes
        print("✓ strategic-reevaluation: 3 consecutive fails on path A " "→ inject")
...
def test_after_hook_receives_is_error_flag() -> None:
    ...
    try:
        ...
        mod = next(
            m for n, m in sys.modules.items() if n.startswith("pyagent_plugin_errflag")
        )
