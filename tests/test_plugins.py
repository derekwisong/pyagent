...
Run with:
    ...
    .venv/bin/python -m tests.test_plugins
"""
...
def _write_plugin(
    ...
    *,
    ...
    tools_line = "tools = [" + ", ".join(f'"{t}"' for t in (provides_tools or [])) + "]"
    sections_line = (
        "prompt_sections = ["
...
def _isolated_config_dir() -> tuple[Path, callable]:
    ...
    so the bundled `memory` plugin doesn't appear in test fixtures by
    default. Tests that want bundled plugins enabled can overwrite the
    config file.
...
def test_hook_failure_isolation() -> None:
    ...
    try:
        ...
        assert (
            plugin_module.get_state()["seen"] == "hello"
...
def test_memory_round_trip() -> None:
    ...
    try:
        ...
        assert upd == "updated stack_choices.md: body", upd
        ...
        assert "USER" in u, u
        assert (
            cfg / "plugins" / "memory" / "USER.md"
        ).read_text() == "prefers tabs over spaces\n"
        ...
        print(
            "✓ memory round trip: create_memory / read_memory / update_memory / write_user"
        )
    finally:
        restore()
...
def test_recall_memory() -> None:
    ...
    semantically-matching file. Skipped unless PYAGENT_HEAVY_TESTS=1
    (triggers a ~130MB ONNX model download from HuggingFace on first
    run, so not appropriate for default CI)."""
    if not os.environ.get("PYAGENT_HEAVY_TESTS"):
        print("⊘ PYAGENT_HEAVY_TESTS unset; skipping recall test")
        return
    try:
    ...
    try:
        ...
        first_hit_line = next(ln for ln in result.splitlines() if "memories/" in ln)
...
def test_create_memory() -> None:
    ...
    try:
        ...
        bad_title = create_memory(category="Style", title="line\nbreak", content="x")
...
def test_update_memory() -> None:
    ...
    try:
        ...
        index_path = cfg / "plugins" / "memory" / "MEMORY.md"
        body_path = cfg / "plugins" / "memory" / "memories" / "uv_choice.md"
        ...
        bad = update_memory(filename="uv_choice.md", description="line\nbreak")
        assert "description contains a newline" in bad, bad

        # Newline rejection in category.
        bad_cat = update_memory(filename="uv_choice.md", category="A\n## B")
        assert "category contains a newline" in bad_cat, bad_cat

        # Bad filename rejected.
        invalid = update_memory(filename="../escape.md", description="x")
...
def test_read_memory_strips_frontmatter() -> None:
    ...
    try:
        ...
        assert out.startswith("[created 2026-05-04T08:00:00+00:00]\n\n"), out[:80]
...
def test_delete_memory_orphan_tolerant() -> None:
    ...
    try:
        ...
        body_path = cfg / "plugins" / "memory" / "memories" / "orphan_bullet.md"
        body_path.unlink()
        ...
        index_text = "\n".join(
            ln for ln in index_text.splitlines() if "orphan_body.md" not in ln
        )
...
def test_update_memory_anchored_match() -> None:
    ...
    try:
        ...
        related_lines = [ln for ln in index.splitlines() if "related.md" in ln]
...
def test_update_memory_per_key_frontmatter_merge() -> None:
    ...
    try:
        ...
        body_path = cfg / "plugins" / "memory" / "memories" / "fm_merge.md"
        original = body_path.read_text()
        original_created = original.split("\n", 2)[1]  # `created_at: ...`
...
def test_recall_memory_surfaces_category() -> None:
    ...
    Skipped unless PYAGENT_HEAVY_TESTS=1 (fastembed model download)."""
    if not os.environ.get("PYAGENT_HEAVY_TESTS"):
        print("⊘ PYAGENT_HEAVY_TESTS unset; skipping recall category test")
        return
    try:
        import fastembed  # noqa: F401
    except ImportError:
...
def test_recall_memory_temporal_filter() -> None:
    ...
    and any without created_at frontmatter. Skipped unless
    PYAGENT_HEAVY_TESTS=1 (fastembed model download)."""
    if not os.environ.get("PYAGENT_HEAVY_TESTS"):
        print("⊘ PYAGENT_HEAVY_TESTS unset; skipping recall temporal test")
        return
    try:
        import fastembed  # noqa: F401
    except ImportError:
...
def test_atomic_write_helper() -> None:
    ...
    try:
        ...
        assert not (
            tmpd / "MEMORY.md.tmp"
        ).exists(), "tmp file should be gone after replace"
...
def test_write_session_attachment_no_session() -> None:
    ...
    try:
        ...
        assert (
            plugin_module.get_state()["path"] is None
...
def test_graceful_degradation_when_memory_disabled() -> None:
    ...
    try:
        ...
        assert loaded.declared_tool_provenance.get("read_memory") == "memory"
        err = plugins_mod.format_missing_tool_error(
            name="read_memory",
            available=["read_file", "grep"],
            declared_tool_provenance=loaded.declared_tool_provenance,
