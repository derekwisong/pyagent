...
introduced (test_session_replay enforces that invariant).

No subprocess, no network. Run with:
    .venv/bin/python -m tests.test_read_file_ceiling
"""
...
def _check_small_read_file_inline() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="pyagent-test-ceiling-") as t:
...
def _check_large_read_file_offloads() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="pyagent-test-ceiling-") as t:
        ...
        size = 17_500
        assert threshold < size < ceiling, f"sanity: {threshold} < {size} < {ceiling}"
        text = "y" * size
        rendered = agent._render_tool_result("read_file", text)
        assert rendered.startswith("[offload "), rendered
        assert text not in rendered, "raw payload leaked through soft-threshold offload"
...
def _check_read_file_coerces_string_args() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="pyagent-test-coerce-") as t:
