...
No subprocess, no network. Run with:
    .venv/bin/python -m tests.test_attachment_lru
"""
...
def _check_under_cap_no_eviction() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="pyagent-lru-") as t:
        session = Session(session_id="under", root=Path(t), attachment_dir_cap_mb=5)
        paths = []
        for i in range(4):
...
def _check_over_cap_oldest_atime_evicted() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="pyagent-lru-") as t:
        ...
        session = Session(session_id="over", root=Path(t), attachment_dir_cap_mb=3)
...
def _check_cap_zero_disables_eviction() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="pyagent-lru-") as t:
        session = Session(session_id="off", root=Path(t), attachment_dir_cap_mb=0)
        paths = []
        # Write 5 MB worth of data; with cap disabled, all stays.
        for _ in range(5):
            paths.append(session.write_attachment("read_file", "q" * 1_000_000))
        for p in paths:
...
def _check_config_wiring() -> None:
    ...
    assert cap_mb == 5
    with tempfile.TemporaryDirectory(prefix="pyagent-lru-") as t:
        session = Session(session_id="cfg", root=Path(t), attachment_dir_cap_mb=cap_mb)
...
def _check_validation_warns_and_falls_back() -> None:
    ...
    try:
        ...
        assert any(
            "integer" in r.getMessage() for r in handler.records
        ), handler.records
    finally:
...
def _check_path_a_contract() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="pyagent-lru-") as t:
        ...
        session = Session(session_id="path-a", root=Path(t), attachment_dir_cap_mb=1)
