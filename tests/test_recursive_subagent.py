...
its own subagent) is verified by the manual real-API test; this is
the pure-IPC layer.

Run with:

    .venv/bin/python -m tests.test_recursive_subagent
"""
...
def main() -> None:
    ...
    try:
        ...
        while cli_end.poll(0.05):
            cli_end.recv()
        x_test_end.send({"type": "ready", "agent_id": y_sid})
        deadline = time.monotonic() + 3.0
        seen_ready = False
        while time.monotonic() < deadline:
            if cli_end.poll(0.1):
        ...
        assert (
            x_rq is not None and x_rq.empty()
        ...
        with state._subagent_lock:
            assert (
                y_sid not in state._descendants
