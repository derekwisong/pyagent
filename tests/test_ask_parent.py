...
In-process — no real LLM, no subprocesses. Run with:
    ...
    .venv/bin/python -m tests.test_ask_parent
"""
...
def _make_subagent_state(
    ...
    agent_proc._ChildState, multiprocessing.connection.Connection, threading.Thread
]:
...
def main() -> None:
    tmp = Path(tempfile.mkdtemp(prefix="pyagent-ask-test-"))
    os.chdir(tmp)
    print(f"cwd: {tmp}")
    ...
    try:
        ...
        with pstate._ask_lock:
            assert pstate._inbound_ask_sid == {
                "req-deadbeef": fake_sid
        ...
        result = reply_tool(
            "req-deadbeef", "go ahead, use --break-system-packages? no."
        )
