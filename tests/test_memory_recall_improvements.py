...
Run with:
    .venv/bin/python -m tests.test_memory_recall_improvements
"""
...
def _check_filename_search_terms() -> None:
    ...
    assert (
        _filename_search_terms("stack_choices.md") == "stack choices"
    ), _filename_search_terms("stack_choices.md")
    assert (
        _filename_search_terms("client_naming_convention.md")
...
def _check_gather_chunks_includes_filename() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="pyagent-mem-recall-") as t:
        ...
        # it index.
        try:
            (data_dir / "plugins" / "memory").mkdir(parents=True, exist_ok=True)
            api = _FakeAPI()
            ...
            # both the unit (above) and the integration test
