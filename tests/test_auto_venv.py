...
Run with:
    ...
    .venv/bin/python -m tests.test_auto_venv
"""
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                assert found is None, f"discover should ignore VIRTUAL_ENV; got {found}"
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if in_venv:
                ...
                print(
                    "✓ python_env(agent) reports active venv: " f"{data['venv_path']}"
                )
            else:
                assert data["venv_path"] == "", data
                assert data["exists_before_call"] is False, data
            ...
            assert not (
                ws / ".venv"
            ).exists(), "python_env(agent) should not touch the workspace"
