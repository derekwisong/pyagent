...
Run with:
    ...
    .venv/bin/python -m tests.test_claude_code_cli
"""
...
PLUGIN_DIR = (
    Path(__file__).resolve().parent.parent / "pyagent" / "plugins" / "claude_code_cli"
)
...
class _StubAPI:
    ...
    def register_tool(self, name: str, fn, *, role_only: bool = False) -> None:
        self.tools[name] = fn
...
def test_oversize_json_schema_rejected() -> None:
    ...
    out = tool(prompt="hi", output_format="json", json_schema=huge)
...
def test_oversize_context_file_rejected(tmp_path: Path | None = None) -> None:
    ...
    tmp = Path(tempfile.mkdtemp(prefix="test-claude-cli-"))
    try:
        big = tmp / "big.txt"
