"""End-to-end test for the agent subprocess.
...
in tests.test_permission_handler — exercising them end-to-end would
require a richer stub LLM than ships in the package.

Run with:

    .venv/bin/python -m tests.test_subprocess
"""
...
def main() -> None:
    tmp = Path(tempfile.mkdtemp(prefix="pyagent-test-"))
    os.chdir(tmp)
    print(f"cwd: {tmp}")
