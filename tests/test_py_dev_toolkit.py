...
plugin test suite. The plugin's own missing-tool error path is
...
def _setup() -> tuple[dict, Path]:
    ...
    workdir = Path(tempfile.mkdtemp(prefix="pydevtools_test_"))
...
def _check_lint_findings_and_clean(tools: dict, workdir: Path) -> None:
    ...
    _check(
        "lint summary line includes count",
...
def _check_run_pytest_basic(tools: dict, workdir: Path) -> None:
    ...
    if "pytest-json-report" in out and "<error:" in out:
        _check("pytest test skipped (json-report plugin missing)", True)
        return
    _check(
