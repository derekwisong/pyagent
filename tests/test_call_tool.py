...
Run with:
    ...
    .venv/bin/python -m tests.test_call_tool
"""
...
def _write_plugin(
    ...
    *,
    ...
    tools_line = "tools = [" + ", ".join(f'"{t}"' for t in provides_tools) + "]"
    manifest = (
        f'name = "{name}"\n'
        f'version = "0.1.0"\n'
...
def test_bad_kwargs_returns_marker() -> None:
    ...
    try:
        ...
        _write_plugin(
            cfg / "plugins",
            dirname="01-a",
            name="plug-bad-kwargs-target",
            provides_tools=["takes_x"],
            plugin_py=plugin_a,
        )
        _write_plugin(
            cfg / "plugins",
            dirname="02-b",
            name="plug-bad-kwargs-caller",
            provides_tools=["bad_kw_caller"],
            plugin_py=plugin_b,
        )
        loaded = plugins_mod.load()
