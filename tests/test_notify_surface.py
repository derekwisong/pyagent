...
pattern as `test_ask_parent.py` and `test_notify.py`.
...
In-process — no real LLM, no subprocesses. Run with:
    ...
    .venv/bin/python -m tests.test_notify_surface
"""
...
def main() -> None:
    tmp = Path(tempfile.mkdtemp(prefix="pyagent-notify-surface-test-"))
    os.chdir(tmp)
    print(f"cwd: {tmp}")
    ...
    # Wire the notes_unread emitter manually since the test skips
    ...
    try:
        ...
        ok = _wait_for(lambda: pagent.pending_async_replies.qsize() >= 4, timeout=3.0)
        assert ok, (
        ...
        print("✓ single-sid peek (since=None): cursor advances to 1")

        # 4. single-sid peek with since="0" — entries with seq > 0.
        out = peek(sid=sid_a, since="0")
        assert "cursor=0]:" in out, out
        ...
        ok = _wait_for(lambda: pagent.pending_async_replies.qsize() >= 64, timeout=5.0)
        assert ok, (
        ...
        print("✓ drain resets unread counters; emitted zeroed snapshot")
        ...
        sub_c.send({"type": "subagent_note", "severity": "alert", "text": "y"})
        ...
        print(
            "✓ _collect_subagent_notes returns structured records "
            "(reusable for /notes follow-up)"
        )
