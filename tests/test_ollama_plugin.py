"""End-to-end test for the bundled `ollama` plugin.
...
Run with:
    ...
    .venv/bin/python -m tests.test_ollama_plugin
"""
...
def _check_default_config_lists_ollama() -> None:
    ...
    tmp = Path(tempfile.mkdtemp(prefix="pyagent-test-ollama-"))
        ...
        client.respond(
            conversation=[
                {"role": "user", "content": "go"},
                {
                    ...
                    "tool_calls": [{"id": "abc", "name": "lookup", "args": {"q": "x"}}],
                },
                {
                    "role": "user",
                    "tool_results": [
...
def _check_dialect_resolves_from_show_payload() -> None:
    ...
    with mock.patch.object(ollama_client_mod.requests, "post", side_effect=fail_post):
        d = fresh.dialect
    _check(
...
def _check_http_error_surfaces_ollama_body() -> None:
    ...
    with mock.patch.object(ollama_client_mod.requests, "post", return_value=fail):
        try:
...
def _check_provider_list_models_hook() -> None:
    ...
    def fake_post(url, json=None, timeout=None, stream=False):
        ...
        if "llama3.2" in name:
            return _FakeResponse({"capabilities": ["completion", "tools"]})
        if "llava" in name:
            return _FakeResponse({"capabilities": ["completion", "vision"]})
...
def _check_no_tools_auto_retry() -> None:
    ...
    # retry every error.
    other_error = _FakeResponse(payload={"error": "model not found"}, status_code=404)
...
def _check_temperature_lands_in_request_body() -> None:
    ...
    client = ollama_client_mod.OllamaClient(model="llama3.2", host="http://h:1")
    with (
        mock.patch.object(ollama_client_mod.requests, "post", side_effect=fake_post),
        mock.patch.object(
            ollama_client_mod._config,
            "load",
...
def main() -> None:
    ...
    print("test_ollama_plugin: all checks passed")
