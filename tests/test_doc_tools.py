"""End-to-end test for the doc-tools plugin.
...
Run with:
    ...
    .venv/bin/python -m tests.test_doc_tools
"""
...
def _check_register_publishes_both_tools() -> None:
    cap = _capture_tools()
    assert set(cap["tools"].keys()) == {"extract_doc", "summarize_doc"}, cap["tools"]
...
def _check_env_var_overrides_config() -> None:
    ...
    try:
        # Env beats config.
        with mock.patch.dict(os.environ, {"PYAGENT_DOC_TOOLS_MODEL": "pyagent/echo"}):
...
def _check_input_validation() -> None:
    ...
    # Missing query
    assert extract("/tmp/anything.txt", "").startswith("<error: query is required")
    # Bad max_chars
...
def _check_schema_validation() -> None:
    ...
    assert (
        "schema" not in out.lower().split("\n", 1)[1].split("extraction request:")[0]
    ), out
...
def _check_permission_denied_marker() -> None:
    ...
    bogus = "/tmp/__doc_tools_test_perm_denied__.txt"
    with mock.patch.object(permissions, "require_access", return_value=False):
...
def _check_nonexistent_file_marker() -> None:
    ...
    bogus = "/tmp/__doc_tools_test_does_not_exist__.txt"
...
def _check_subllm_timeout_marker() -> None:
    ...
    try:
        with mock.patch.object(llms, "get_client", return_value=_SlowClient()):
            t0 = time.time()
            out = extract(path, "anything", model="pyagent/echo")
...
def main() -> None:
    ...
    print("test_doc_tools: all checks passed")
