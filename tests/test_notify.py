...
In-process — no real LLM, no subprocesses. Run with:
    ...
    .venv/bin/python -m tests.test_notify
"""
...
def main() -> None:
    tmp = Path(tempfile.mkdtemp(prefix="pyagent-notify-test-"))
    os.chdir(tmp)
    print(f"cwd: {tmp}")
    ...
    try:
        ...
        assert any(
            e.get("type") == "subagent_note" and e.get("agent_id") == fake_sid
            for e in forwarded
        ), forwarded
        ...
        assert (
            pagent.pending_async_replies.qsize() == 0
        ...
        assert any(
            e.get("type") == "subagent_note" and e.get("text") == "from a grandchild"
            for e in forwarded
        ), forwarded
