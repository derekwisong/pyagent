...
Run with:
    ...
    .venv/bin/python -m tests.test_prompt_toolkit
"""
...
def main() -> None:
    ...
    try:
        ...
        # requests". Replaces the old `/queue` test now that the
        # local input queue is gone (issues #68/#69).
        os.write(fd, b"/perms\r")
        slash_out = _read_until(
            fd,
            b"no pending permission requests",
            timeout_s=5.0,
        )
        assert (
            b"no pending permission requests" in slash_out
        ...
        assert b"Traceback" not in combined, f"traceback in PTY output:\n{combined!r}"
        ...
        if os.WIFEXITED(status):
            ...
            raise AssertionError(f"CLI was killed by signal {os.WTERMSIG(status)}")
