...
Run with:
    ...
    .venv/bin/python -m tests.test_skill_eviction
"""
...
def _check_default_no_eviction() -> None:
    ...
    agent.conversation = [
        _user("hello"),
        _assistant(
            text="", tool_calls=[{"id": "c1", "name": "grep", "args": {"pattern": "x"}}]
        ),
    ...
    assert found == grep_payload, f"non-evictable content was mutated: {found!r}"
...
def _check_evict_after_next_assistant_turn() -> None:
    ...
    agent.conversation = [
        ...
        _assistant(
            text="",
            tool_calls=[{"id": "c1", "name": "read_skill", "args": {"name": "foo"}}],
        ),
        _tool_results([{"id": "c1", "name": "read_skill", "content": body}]),
...
def _check_most_recent_preserved() -> None:
    ...
    agent.conversation = [
        ...
        _tool_results([{"id": "c1", "name": "read_skill", "content": body_1}]),
        _assistant(text="now read second"),  # consumes body_1
        _user("ok"),
        _assistant(
            text="",
            tool_calls=[{"id": "c2", "name": "read_skill", "args": {"name": "bar"}}],
        ),
        _tool_results([{"id": "c2", "name": "read_skill", "content": body_2}]),
...
def _check_resume_jsonl_untouched() -> None:
    ...
    tmp = Path(tempfile.mkdtemp(prefix="pyagent-test-evict-resume-"))
    ...
    # append_history stamps a `ts` on each entry at write time; strip
    # before comparing so the round-trip assertion is meaningful.
    loaded_stripped = [
        {k: v for k, v in e.items() if k != "ts"} if isinstance(e, dict) else e
        for e in loaded_back
    ]
    assert (
        loaded_stripped == entries
    ...
    # round-trip identically (ignoring the `ts` stamp added at write).
    ...
    assert (
        on_disk_stripped == entries
    ), "on-disk JSONL no longer matches what was written"
...
def _check_multiple_skills_in_sequence() -> None:
    ...
    agent.conversation = [
        ...
        _assistant(
            text="",
            tool_calls=[{"id": "c1", "name": "read_skill", "args": {"name": "a"}}],
        ),
        _tool_results([{"id": "c1", "name": "read_skill", "content": body_1}]),
        ...
        _assistant(
            text="",
            tool_calls=[{"id": "c2", "name": "read_skill", "args": {"name": "b"}}],
        ),
        _tool_results([{"id": "c2", "name": "read_skill", "content": body_2}]),
        ...
        _assistant(
            text="",
            tool_calls=[{"id": "c3", "name": "read_skill", "args": {"name": "c"}}],
        ),
        _tool_results([{"id": "c3", "name": "read_skill", "content": body_3}]),
